...
async function gotoAndSelectSession(
  page: any,
...
extendedTest.describe('PTY Buffer readRaw() Function', () => {
  ...
  extendedTest('should expose raw buffer data via API endpoint', async ({ page, server, api }) => {
    await clearAllSessions(api)
    const sessionId = await createSession(api, {
      command: 'bash',
      args: ['-c', 'printf "api\ntest\ndata\n"'],
  ...
  extendedTest(
    ...
    async ({ page, server, api }) => {
      ...
      await gotoAndSelectSession(page, server, 'ANSI test session for plain buffer endpoint', 5000)
