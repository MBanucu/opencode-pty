...
export function App() {
    ...
    wsRef.current.onmessage = (event) => {
      try {
        ...
        if (message.type === 'data') {
          logger.info(
            '[Browser] Checking data message, sessionId:',
            message.sessionId,
            'activeSession.id:',
        ...
        if (message.type === 'data' && message.sessionId === activeSessionRef.current?.id) {
          logger.info(
            '[Browser] Received live data for active session:',
            message.sessionId,
            'data length:',
            message.data.length,
            'activeSession.id:',
  ...
  const handleSessionClick = useCallback(async (session: Session) => {
    ...
    debugDiv.style.cssText =
      'position: fixed; top: 0; left: 0; background: red; color: white; padding: 5px; z-index: 9999; font-size: 12px;'
    ...
    try {
      ...
      try {
        ...
        if (response.ok) {
          ...
          if (debugDiv)
            debugDiv.textContent = `LOADED ${outputData.lines?.length || 0} lines: ${session.id}`
        } else {
